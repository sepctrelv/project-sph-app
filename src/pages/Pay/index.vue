...
export default {
  ...
  methods: {
    async getPaymentInfo() {
      const result = await this.$API.reqGetPaymentInfo(this.orderId);
      if (result.code === 200) {
    ...
    async handlePay() {
      // 生成二维码
      let result = await QRCode.toDataURL("Hello World");
      MessageBox.alert(`<img src='${result}' alt="" />`, "请支付", {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          // 发请求获取用户支付状态
          let result = await this.$API.reqGetPaymentStatus(this.orderId);
          console.log(result);
          if (result.code === 200) {
            clearInterval(this.timer);
            this.timer = null;
