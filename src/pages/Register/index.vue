    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <input
          ...
          v-model.trim="phone"
        />
        <span class="error-msg" v-show="check.phone">请输入正确的手机号！</span>
      </div>
      <div class="content">
        <label>验证码:</label>
        <input type="text" placeholder="请输入验证码" v-model.trim="code" />
        <button class="btn-verification-code" @click="getCode">
          获取验证码
        </button>
        <span class="error-msg" v-show="check.code">验证码不能为空！</span>
      </div>
      <div class="content">
        <label>登录密码:</label>
        <input
        ...
        />
        <span class="error-msg" v-show="check.password">密码不能为空！</span>
      </div>
      <div class="content">
        <label>确认密码:</label>
        <input
        ...
        />
        <span class="error-msg" v-show="check.confirmPassword"
          >两次密码不一致！</span
        >
      </div>
      <div class="controls">
        ...
        <span class="error-msg" v-show="check.agree">请先勾选同意协议！</span>
      </div>
      <div class="btn">
...
export default {
  ...
  methods: {
    ...
    checkInfo() {
      ...
      // 是否勾选协议
      if (!this.agree) {
        this.check.agree = true;
        isChecked = false;
      } else {
        this.check.agree = false;
      }
      // 手机号是否等于11位
