  ...
  <div class="cart-complete-wrap">
    <div class="cart-complete">
      ...
      <div class="goods">
        <div class="left-good">
          <div class="left-pic">
            <img :src="skuInfo.skuDefaultImg" :alt="skuInfo.skuName" />
          </div>
          <div class="right-info">
            <p class="title">{{ skuInfo.skuName }}</p>
            <p class="attr">
              <span v-for="(item, index) in spuSaleAttrList" :key="item.id"
                >{{ index === 0 ? "" : " / " }}{{ item.saleAttrName }}:
                {{ getSaleAttrValue(item.spuSaleAttrValueList) }}</span
              >
              数量：{{ skuNum }}
            </p>
          </div>
